...
(function (global, factory) {
    ...
    let WINDOW = _WINDOW, DOCUMENT = _DOCUMENT, IS_BROWSER = !!WINDOW.document;
    ...
    function report(_ref) {
        let timedOutTests = {};
        for (let key in _ref.nodesFound) {
            if (!(_ref.nodesTested.conflict[key] || _ref.nodesTested.noConflict[key])) timedOutTests[key] = _ref.nodesFound[key];
        }
        let conflictsCount = Object.keys(_ref.nodesTested.conflict).length;
        if (conflictsCount > 0) {
            console.info("%cConflict".concat(conflictsCount > 1 ? 's' : '', " found:"), 'color: darkred; font-size: large');
            let data = {};
            for (let _key in _ref.nodesTested.conflict) {
                let item = _ref.nodesTested.conflict[_key];
                data[_key] = {
        ...
        let noConflictsCount = Object.keys(_ref.nodesTested.noConflict).length;
        if (noConflictsCount > 0) {
            console.info("%cNo conflict".concat(noConflictsCount > 1 ? 's' : '', " found with ").concat(noConflictsCount === 1 ? 'this' : 'these', ":"), 'color: green; font-size: large');
            let _data = {};
            for (let _key2 in _ref.nodesTested.noConflict) {
                let _item = _ref.nodesTested.noConflict[_key2];
                _data[_key2] = {
    ...
    let md5 = createCommonjsModule(function (module) {
        (function ($) {
            ...
            function rstrHMACMD5(key, data) {
                let i, bkey = rstr2binl(key), ipad = [], opad = [], hash;
            ...
            /**
             * Calculates MD5 value for a given string. If a key is provided, calculates the HMAC-MD5 value. Returns a
             * Hex encoded string unless the raw argument is given.
             *
            ...
            if (module.exports) module.exports = md5;
            else $.md5 = md5;
        })(commonjsGlobal);
    });
    function md5ForNode(node) {
        if (null === node || 'object' !== _typeof(node)) return undefined;
        if (node.src) return md5(node.src);
        if (node.href) return md5(node.href);
        // eslint-disable-line yoda
        if (node.innerText && '' !== node.innerText) return md5(node.innerText);
        return undefined;
    }
    let diagScriptId = 'fa-kits-diag', nodeUnderTestId = 'fa-kits-node-under-test';
    let md5Attr = 'data-md5', detectionIgnoreAttr = 'data-fa-detection-ignore';
    let timeoutAttr = 'data-fa-detection-timeout', resultsCollectionMaxWaitAttr = 'data-fa-detection-results-collection-max-wait';
    ...
    function pollUntil(_ref) {
        let _ref$fn = _ref.fn,
            fn = _ref$fn === void 0 ? function () { return true; } : _ref$fn,
        ...
        return new Promise(function (resolve, reject) {
            ...
            function poll(duration, cumulativeDuration) {
                setTimeout(function () {
                    let result = fn();
                    if (showProgress) console.info(progressIndicator);
                    // eslint-disable-line no-extra-boolean-cast
                    if (!!result) resolve(result);
                    else {
                        let nextDuration = 250, nextCumulativeDuration = nextDuration + cumulativeDuration;
                        if (nextCumulativeDuration <= maxDuration) poll(nextDuration, nextCumulativeDuration);
                        else reject('timeout'); // eslint-disable-line prefer-promise-reject-errors
                    }
                }, duration);
            }
    ...
    function detectWebfontConflicts() {
        let linkTags = Array.from(DOCUMENT.getElementsByTagName('link')).filter(function (t) { return !t.hasAttribute(detectionIgnoreAttr); });
        let styleTags = Array.from(DOCUMENT.getElementsByTagName('style')).filter(function (t) {
            if (t.hasAttribute(detectionIgnoreAttr)) return false;
            ...
            return !(WINDOW.FontAwesomeConfig && t.innerText.match(new RegExp("svg:not\\(:root\\)\\.".concat(WINDOW.FontAwesomeConfig.replacementClass))));
        });
        function runDiag(scriptOrLinkTag, md5) {
            ...
            let testIconId = 'fa-test-icon-' + md5, iTag = DOCUMENT.createElement('i');
            iTag.setAttribute('class', 'fa fa-coffee');
            iTag.setAttribute('id', testIconId);
            let diagScript = DOCUMENT.createElement('script');
            diagScript.setAttribute('id', diagScriptId);
            // WARNING: this function will be toString()'d and assigned to innerText of the diag script/ element that
            // we'll be putting into a diagnostic iframe. That means that this code won't compile until after the outer
            // script has run and injected this code into the iframe. There are some compile time errors that might
            // occur there. For example, using single line (double-slash) comments like this one inside that function
            // will probably cause it to choke. Chrome will show an error like this: Uncaught SyntaxError: Unexpected
            // end of input
            let diagScriptFun = function diagScriptFun(nodeUnderTestId, testIconId, md5, parentOrigin) {
                parent.FontAwesomeDetection.__pollUntil({
                    fn: function fn() {
                        let computedStyle = window.getComputedStyle(document.getElementById(testIconId));
                        let fontFamily = computedStyle.getPropertyValue('font-family');
                        return !!fontFamily.match(/FontAwesome/) || !!fontFamily.match(/Font Awesome [56]/);
                    }
                }).then(function () {
                    let node = document.getElementById(nodeUnderTestId);
                    parent.postMessage({type: 'fontawesome-conflict', technology: 'webfont', href: node.href, innerText: node.innerText, tagName: node.tagName, md5: md5}, parentOrigin);
                }).catch(function (e) {
                    let node = document.getElementById(nodeUnderTestId);
                    if (e === 'timeout') parent.postMessage({type: 'no-conflict', technology: 'webfont', href: node.src, innerText: node.innerText, tagName: node.tagName, md5: md5}, parentOrigin);
            ...
            domready(function () { return DOCUMENT.body.appendChild(diagFrame); });
        }
        let cssByMD5 = {};
    ...
    function detectSvgConflicts(currentScript) {
        let scripts = Array.from(DOCUMENT.scripts).filter(function (t) { return !t.hasAttribute(detectionIgnoreAttr) && t !== currentScript; });
        let scriptsByMD5 = {};
        let _loop = function _loop(scriptIdx) {
            ...
            if (scripts[scriptIdx].src !== '') scriptUnderTest.src = scripts[scriptIdx].src;
            ...
            let diagScriptFun = function diagScriptFun(nodeUnderTestId, md5, parentOrigin) {
                parent.FontAwesomeDetection.__pollUntil({
                    fn: function fn() { return !!window.FontAwesomeConfig || !!window.FontAwesomeKitConfig; }
                }).then(function () {
                    let scriptNode = document.getElementById(nodeUnderTestId);
                    parent.postMessage({type: 'fontawesome-conflict', technology: 'js', src: scriptNode.src, innerText: scriptNode.innerText, tagName: scriptNode.tagName, md5: md5}, parentOrigin);
                }).catch(function (e) {
                    let scriptNode = document.getElementById(nodeUnderTestId);
                    if (e === 'timeout') parent.postMessage({type: 'no-conflict', src: scriptNode.src, innerText: scriptNode.innerText, tagName: scriptNode.tagName, md5: md5}, parentOrigin);
                    else console.error(e);
                });
            };
    ...
    function setDoneResults(_ref2) {
        ...
        WINDOW.FontAwesomeDetection.nodesTested = _ref2.nodesTested;
        WINDOW.FontAwesomeDetection.nodesFound = _ref2.nodesFound;
    ...
    function conflictDetection() {
        ...
        let nodesTested = {conflict: {}, noConflict: {}};
        WINDOW.onmessage = function (e) {
            if (WINDOW.location.origin === 'file://' || e.origin === WINDOW.location.origin) {
                if (e && e.data) {
                    if (e.data.type === 'fontawesome-conflict') nodesTested.conflict[e.data.md5] = e.data;
                    else if (e.data.type === 'no-conflict') nodesTested.noConflict[e.data.md5] = e.data;
                }
            }
        };
        let scriptsToTest = detectSvgConflicts(DOCUMENT.currentScript), cssToTest = detectWebfontConflicts();
        let nodesFound = _objectSpread2(_objectSpread2({}, scriptsToTest), cssToTest);
        let testCount = Object.keys(scriptsToTest).length + Object.keys(cssToTest).length;
        // The resultsCollectionMaxWait allows for the time between when the tests running under child iframes call
        // postMessage with their results, and when the parent window receives and handles those events with window.onmessage.
        // Making it configurable allows us to test the scenario where this timeout is exceeded. Naming it something very
        // different from "timeout" is to help avoid the potential ambiguity between these two timeout-related settings.
        ...
        if (testCount === 0) {
            ...
            pollUntil({
                ...
                console.info('DONE!');
                setDoneResults({nodesTested: nodesTested, nodesFound: nodesFound});
                report$$1({nodesTested: nodesTested, nodesFound: nodesFound});
                console.groupEnd();
            }).catch(function (e) {
                if (e === 'timeout') {
    ...
    WINDOW.FontAwesomeDetection = _objectSpread2(_objectSpread2(_objectSpread2({}, _default), initialConfig), {}, {
    ...
    let PREFIX_TO_STYLE = familyProxy((_familyProxy = {}, _defineProperty(_familyProxy, FAMILY_CLASSIC, {
        ...
        'fa-light': 'light', 'fat': 'thin', 'fa-thin': 'thin', 'fad': 'duotone', 'fa-duotone': 'duotone', 'fab': 'brands',
        'fa-brands': 'brands', 'fak': 'kit', 'fa-kit': 'kit'
    ...
    bunker(function () {
        if (IS_BROWSER && IS_DOM) conflictDetection(window.FontAwesomeDetection.report);
    });
})));